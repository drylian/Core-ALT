...
const fs = require('fs')
const { config, event, ctable, db } = require('alter');
...
function loadSlashRest(client) {
  ...
  const commandFolders = readdirSync('./discord/SlashFolder');
  for (const folder of commandFolders) {
    const commandFiles = readdirSync(join('./discord/SlashFolder', folder)).filter(file => file.endsWith('.js'));
  ...
  const table = ctable(tableTitle, tableHeaders, tableRows, 'red');
  console.log(table);
...
function loadSlashCommands(client) {
  ...
  const commandFolders = readdirSync(`./discord/SlashFolder`);
  for (const folder of commandFolders) {
    const commandFiles = readdirSync(`./discord/SlashFolder/${folder}`).filter(file => file.endsWith('.js'));
    for (const file of commandFiles) {
      const command = require(`../SlashFolder/${folder}/${file}`);
      if (command.data.name) {
        client.slash.set(command.data.name, command);
