...
async function setup(configAtual) {
    ...
    `);
		...
		const askQuestion = (index) => {
			...
			if (configAtual[key]) {
				process.stdout.write(question + `(atual:${configAtual[key]}):`);
			} else if (!padrao) {
			...
			process.stdin.once('data', (data) => {
				...
				let valueToSet
				if (configAtual[key]) {
					valueToSet = input.length > 0 ? input : configAtual[key];
				} else {
					valueToSet = input.length > 0 ? input : padrao; // Usa o valor padrão se nenhum valor for inserido
				}

				if (!valueToSet && !padrao && !configAtual[key]) {
					console.clear();
