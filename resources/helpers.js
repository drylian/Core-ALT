...
export const cleanDirectoryPath = (path) => path.replace(/(\/(\/*))|(^$)/g, '/');

export function fileBitsToString(mode, directory) {
    ...
    if (buf.length === 0) {
        // If the file is a directory, make sure it has the directory flag.
        if (directory) {
            buf = 'd';
        } else {
            buf = '-';
 ...
 * This allows using the path as part of a URL while preserving the slashes.
