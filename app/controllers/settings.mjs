import { jsonsv, json } from "../utils/json.mjs";
import { dirCR, dirEX } from "../utils/folder.mjs";
import root from "./settings/Default.mjs";
import { controller } from "./loggings.mjs";
import { gen } from "../utils/gerate.mjs";
...
async function Settings() {
	try {
		...
		const cs = json(root.configPATH + "/loggings.json");
		 ...
		 */
		c = {};c = json(root.configPATH + "/settings.json"); // Rele o json e atualiza os valores atuais
		...
		core("sys", "Erro ao tentar fazer as verificações do painel: " + err.stack);
		return err;
	}
