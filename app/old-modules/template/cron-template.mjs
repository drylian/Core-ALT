...
/**************************************************************/
    ...
    const { log } = await import('armazenamento/Controllers')
    const path = './app/storage/Csrf-tokens';

    fs.readdir(path, (err, files) => {
        if (err) {
            log('Erro ao ler diretório:'+ err);
            return;
        }
        ...
        files.forEach((file) => {
            ...
            fs.stat(filePath, (err, stats) => {
                if (err) {
                    log('Erro ao obter informações do arquivo:'+ err);
                    return;
                }
                ...
                if (timeDifference > tenHoursInMilliseconds) {
                    fs.unlink(filePath, (err) => {
                        if (err) {
                            log('Erro ao excluir arquivo:'+ err);
                            return;
                        }
                        log('Arquivo excluído:'+ filePath);
                    });
                }
            });
        });
